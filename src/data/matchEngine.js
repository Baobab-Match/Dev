...
const FIELD_CLIMATE_FIT = {
  ...
  "기후성 질병 및 보건의료":      { 폭염: 0.8, 홍수: 0.8 }, // 폭염→온열질환, 홍수→수인성질병(콜레라 등) 모두 강한 연관
};
...
const TECH_FIELD_KEYWORDS = {
  "가뭄 대응": [
  "가뭄", "관개", "관개시설", "점적관개", "저수지", "저수", "빗물",
  "우수저류", "우수저류조", "절수", "토양보습", "한발", "가뭄대응",
  ],
  "재생에너지 및 에너지 전환": [
  ...
  "사회적 갈등 및 강제이주": [
  "난민", "강제이주", "이주민", "실향민", "분쟁", "갈등", "평화",
  "재정착", "난민캠프", "인도적지원", "사회통합", "구호",
  ],
  // 초안 키워드 — 한아프리카재단 250대기업 어휘로 추후 보강 필요
  "기후성 질병 및 보건의료": [
    "보건", "의료", "병원", "진단", "진단키트", "백신", "의약품", "제약",
    "헬스케어", "원격의료", "감염병", "방역", "말라리아", "수인성질병",
    "의료기기", "보건소",
  ],
};
...
// 2026-08: 기준선을 감이 아니라 데이터로 재보정 — AI 매칭(baobab_scores_3_match.ipynb
// 셀 32)에서 54개국×11개분야×3개유형(company는 export_exp 포함) 전체 2,376개 조합의
// 실제 matchScore 분포를 백분위로 뽑아서 정함 (상위 20%/50%/80% 컷).
// 예전 75/60/45는 규칙기반 엔진 초기 설계 당시 감으로 잡은 값이라 실제 분포와
// 맞는지 검증된 적이 없었음 — 이번에 AI·규칙기반 양쪽에 동일 기준으로 통일.
export function matchTier(score) {
  if (score >= 58.1) return { tier: "strong", tierNote: "조건에 매우 잘 맞는 국가입니다." };
  if (score >= 45.6) return { tier: "good",   tierNote: "여러 조건에서 적합한 국가입니다." };
  if (score >= 26.1) return { tier: "fair",   tierNote: "일부 조건에서 적합한 국가입니다." };
